...
// Flush the queue after transmission
int dequeue() {
    pthread_mutex_lock(&transmission_queue_lock);
...
// Simulated transmission to cloud server
bool write_data_to_cloud() {
    ...
    if (file == NULL) {
        fprintf(stderr, "Failed to open transmission file\n");
        return false;
    }
    ...
    for (unsigned int i = 0; i < count; i++) {
        fprintf(file, "    {\"device_id\": \"%s\", \"timestamp\": \"%s\", \"power_watts\": %.2f}",
                data_transmission_queue[i].device_name, data_transmission_queue[i].timestamp, data_transmission_queue[i].power);
